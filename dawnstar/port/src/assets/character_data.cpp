...
CharacterData CharacterData::Load(DatArchive& archive) {
    CharacterData data;
    BinaryReader in = archive.OpenResource("charin.dat");
    ...
    // Player.loadCharacterData() throws here rather than proceeding --
    // the 13+2*skillCount class-template row width below is only correct
    // for the 14 skills every other skill-indexed table in the codebase
