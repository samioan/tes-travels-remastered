...
// geomin.dat (DungeonGeometry) and prints enough to check by hand against
// ../../src/Monster.java/DungeonGenerator.java and
// docs/ASSET_FORMATS.md/CLASS_MAP.md.
#include <cstdio>
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        dawnstar::DungeonGeometry geom = dawnstar::DungeonGeometry::Load(archive);
        std::printf("dungeon geometry rows: %zu\n", geom.rows.size());
        for (int lvl = 1; lvl <= 3 && lvl <= static_cast<int>(geom.rows.size()); ++lvl) {
            const dawnstar::DungeonGeomRow& r = geom.rows[lvl - 1];
