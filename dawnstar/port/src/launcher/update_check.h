...
// Finding out whether a newer build exists. Ported from shadowkey-decomp's
// port/src/launcher/update_check.h/.cpp -- the HTTP/JSON/version-compare
// machinery is unchanged (nothing in it was game-specific), but
// FetchLatestRelease gained a tag-prefix filter: this repository publishes
// releases for three games (dawnstar/oblivion/stormhold) tagged
...
// dawnstar-", not just the newest release in the whole repo -- which is
// all shadowkey-decomp's own single-game repo ever needed.
//
...
struct ReleaseInfo {
    std::string tag;          // "dawnstar-v0.1.0"
    std::string version;      // "0.1.0" -- the tag with the "dawnstar-v" prefix removed
    std::string assetName;    // "DawnstarRemastered-dawnstar-v0.1.0-win64.zip"
...
// `tagPrefix` (e.g. "dawnstar-v") and has a downloadable .zip asset.
