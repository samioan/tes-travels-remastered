...
.DESCRIPTION
    Ported from dawnstar's own port/check_dist.ps1 (itself from
...
# --- layout -----------------------------------------------------------
$launcher = Join-Path $DistDir 'Stormhold.exe'
$game = Join-Path $DistDir 'bin/stormhold_port.exe'
...
if (Test-Path $launcher) {
    ...
    if ($size -lt 400KB) {
        Fail ("Stormhold.exe is only $([math]::Round($size/1KB)) KB -- too small to " +
