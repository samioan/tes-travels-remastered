...
struct GeneratedLevel {
    ...
    // Dungeon.java's `populated` flag -- gates cross-level tile reads
    // (dungeon/dungeon_runtime.h's TileAt) AND move eligibility (player/
    // player_movement.h's CommitMove), matching Player.commitMove()'s own
    // `if (!target.populated) return false;`. Real values since M52
    // (world/game_advancement.h): the hub's own constructor sets
    // `populated=true` unconditionally, matching BuildHubLevel below; a
    // STANDARD level's `populated` starts false (matching Dungeon's own
    // plain field default, and PopulateLevel below) and is only ever
    // flipped true by `GameAdvancement::OpenZone` -- see that class's own
    // header comment for exactly when that happens (and, just as
    // importantly, when it deliberately does NOT). Before M52 this field
    // was an always-true placeholder (every generated level counted as
    // "populated" the instant this port's own eager BuildWorld generated
    // it, since nothing else ever flipped it any other way) -- kept
    // named `populated` throughout that placeholder era too (not
    // `visited`, this field's original M6-era name, to match Dungeon
    // .java's own field directly and avoid "has the PLAYER been here"
