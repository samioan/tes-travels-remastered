...
// rather than a folder to search (see launcher/install.h). M78 restored
// shadowkey-decomp's optional font row: the Nokia 3650's own ROM fonts
// (Ceurope.gdr, plus Browsereur.gdr from beside it), which the game draws
// its text in when present -- see graphics/bitmap_font.h.
//
...
using stormhold::launcher::LauncherConfig;

// ---------------------------------------------------------------------
// Palette. The banner is a mostly-black background behind gold lettering
// ("The Elder Scrolls Travels: Stormhold Remastered"), so a dark panel
...
struct UpdateCheckResult {
    bool ok = false;
    stormhold::launcher::ReleaseInfo release;
    std::string error;
};

struct UpdateProgressMessage {
    stormhold::launcher::UpdateProgress progress;
};
...
struct Impl {
    ...
    stormhold::launcher::Banner banner;
    std::string installRoot;   // where Stormhold_port.exe lives
    ...
    stormhold::launcher::ReleaseInfo availableRelease;
    std::wstring updateNote;
    // Set while an install is in flight. The window refuses to close and
    // the game refuses to start during it -- replacing stormhold_port.exe
...
int HeaderHeightFor(const stormhold::launcher::Banner& banner, int width, int fallback) {
...
int WidthForHeaderHeight(const stormhold::launcher::Banner& banner, int header) {
...
COLORREF SeamColorFor(const stormhold::launcher::Banner& banner) {
...
void Refresh(Impl& impl) {
    impl.dataPath = stormhold::launcher::ResolveAgainst(impl.installRoot, impl.config.gameData);
    impl.dataOk = stormhold::launcher::IsGameDataRoot(impl.dataPath);
    impl.fontPath = stormhold::launcher::ResolveAgainst(impl.installRoot, impl.config.font);
    impl.fontOk = stormhold::launcher::IsUsableFont(impl.fontPath);
...
void Save(Impl& impl) { stormhold::launcher::SaveLauncherConfig(impl.configPath, impl.config); }
...
void StartUpdateCheck(HWND hwnd) {
    std::thread([hwnd]() {
        ...
        result->ok = stormhold::launcher::FetchLatestRelease(
            STORMHOLD_UPDATE_OWNER, STORMHOLD_UPDATE_REPO, STORMHOLD_UPDATE_TAG_PREFIX,
...
void ForwardProgress(const stormhold::launcher::UpdateProgress& progress, void* context) {
...
void StartInstall(HWND hwnd, const std::string& installRoot,
                  const stormhold::launcher::ReleaseInfo& release) {
    std::thread([hwnd, installRoot, release]() {
        auto* done = new UpdateDoneMessage();
        done->tag = release.tag;
        done->ok = stormhold::launcher::InstallUpdate(installRoot, release, ForwardProgress, hwnd,
                                                       done->error);
...
void ChooseGameData(Impl& impl) {
    const std::wstring picked =
        PickPath(impl.hwnd, false, L"Select your Stormhold .jar", L"Stormhold game (*.jar)",
                 L"*.jar");
    ...
    std::string error;
    const bool installed = stormhold::launcher::InstallGameJar(source, destination, error);
    impl.busy = false;
    if (!installed) {
        Refresh(impl);
    ...
    impl.config.gameData = stormhold::launcher::RelativeToIfInside(impl.installRoot, destination);
    Save(impl);
    Refresh(impl);
...
bool InstallFontFrom(Impl& impl, const std::string& source) {
    ...
    std::string error;
    if (!stormhold::launcher::InstallFonts(source, destination, copiedItalic, error)) {
        SetStatus(impl, Widen(error), kBad);
        return false;
    }
    impl.config.font =
        stormhold::launcher::RelativeToIfInside(impl.installRoot, (fs::path(destination) / "Ceurope.gdr").string());
    Save(impl);
    Refresh(impl);
    SetStatus(impl,
...
std::string FindGameExecutable(const std::string& installRoot) {
    const fs::path candidates[] = {
        fs::path(installRoot) / "bin" / "stormhold_port.exe",
        fs::path(installRoot) / "stormhold_port.exe",
    };
...
void Play(Impl& impl) {
    ...
    if (exe.empty()) {
        SetStatus(impl, L"stormhold_port.exe is missing from this folder — the install "
    ...
    // The game reads both of these; see main.cpp. STORMHOLD_USER_DIR keeps
    // saves and the log out of bin/ and in one folder a player can find,
    // and STORMHOLD_SCALE replaces what used to be a hardcoded 2x window.
    SetEnvironmentVariableW(L"STORMHOLD_USER_DIR", Widen(userDir).c_str());
    SetEnvironmentVariableW(L"STORMHOLD_SCALE", std::to_wstring(impl.config.scale).c_str());

    // argv[1] is the asset root, argv[2] the optional device font (M78) --
    // quoted because both routinely contain spaces.
    ...
    if (!CreateProcessW(nullptr, mutableCommand.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW,
        ...
        SetStatus(impl, L"Couldn't start the game. See user\\stormhold_port.log.", kBad);
        return;
    }
...
void PaintWindow(Impl& impl, HDC target) {
    ...
    if (impl.dataOk) {
        ...
        DrawTextLine(dc, impl.valueFont, kBad, margin, impl.P(kRow1Value), valueWidth, impl.S(22),
                     L"Not set — choose your Stormhold .jar to begin.");
    }
    ...
    DrawTextLine(dc, impl.captionFont, RGB(96, 84, 60), margin, impl.P(kVersionY),
                 width - 2 * margin, impl.S(18), L"Stormhold Remastered " STORMHOLD_LAUNCHER_VERSION_W,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_APP_UPDATE_CHECKED: {
            ...
            const int comparison = stormhold::launcher::CompareVersions(
                STORMHOLD_LAUNCHER_VERSION, result->release.version);
            if (comparison < 0) {
        ...
        case WM_APP_UPDATE_PROGRESS: {
            ...
            const stormhold::launcher::UpdateProgress& progress = progressMessage->progress;
            if (progress.stage == stormhold::launcher::UpdateStage::Downloading &&
        ...
        case WM_APP_UPDATE_DONE: {
            ...
            if (stormhold::launcher::RelaunchLauncher(impl->installRoot)) {
                DestroyWindow(hwnd);
            } else {
                impl->updateState = UpdateState::UpToDate;
                impl->updateNote.clear();
                ...
                SetStatus(*impl,
                          L"Updated to " + Widen(done->tag) +
                              L". Close and reopen Stormhold to use it.",
                          kGold);
            }
            return 0;
        }
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int showCommand) {
    ...
    Impl impl;
    impl.installRoot = stormhold::ExecutableDirectory();
    impl.configPath = (fs::path(impl.installRoot) / LauncherConfig::kFileName).string();
    impl.banner = stormhold::launcher::LoadEmbeddedBanner();
    stormhold::launcher::LoadLauncherConfig(impl.configPath, impl.config);
    ...
    windowClass.lpszClassName = L"StormholdLauncherWindow";
    ...
    if (impl.config.font.empty()) {
        const std::string found = stormhold::launcher::FindExistingFont();
    ...
    // whether there is a newer Stormhold build -- on a worker thread,
    // because a slow or unreachable network must never delay the window
    // appearing.
    if (stormhold::launcher::UpdatesEnabledForThisBuild(STORMHOLD_LAUNCHER_VERSION)) {
        stormhold::launcher::CleanUpPreviousUpdate(impl.installRoot);
    ...
    if (autoInstalledFont) {
        ...
        SetStatus(impl,
                  L"Stormhold Remastered doesn't include the game itself. Point it at your "
                  L"own copy of TEST-Stormhold.jar to get started.",
                  kMuted);
    }
