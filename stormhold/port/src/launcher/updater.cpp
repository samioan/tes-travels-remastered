...
bool LooksLikeStormholdPackage(const std::vector<std::string>& entryPaths) {
    ...
    for (const std::string& path : entryPaths) {
        ...
        if (lowered == "stormhold.exe") hasLauncher = true;
        if (lowered == "bin/stormhold_port.exe") hasGame = true;
    }
...
bool InstallUpdate(const std::string& installRoot, const ReleaseInfo& release,
    ...
    // Check before unpacking: an archive that is not a Stormhold build must
    ...
    if (!LooksLikeStormholdPackage(paths)) {
        error = "the downloaded archive does not look like a Stormhold build";
        fs::remove_all(updateDir, code);
        return false;
    }
...
bool RelaunchLauncher(const std::string& installRoot) {
    const fs::path exe = fs::path(installRoot) / "Stormhold.exe";
    std::error_code code;
