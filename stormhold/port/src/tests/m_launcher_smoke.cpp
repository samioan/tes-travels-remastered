...
using namespace stormhold::launcher;
...
// to build/ (two levels above stormhold/), by trying every plausible
// working directory rather than assuming one. Developer builds run this
// from build/ by hand; CI (both ci.yml and release-stormhold.yml) invokes
// the exe by a path from the repo root instead, without cd'ing into
// build/ first -- the same real CWD mismatch dawnstar's own version of
// this test hit in its first CI run, not a hypothetical one.
// `relativeToRepoRoot` is the same path spelled from the repo root, for
// when that's where we actually are.
std::string FindAsset(const std::string& relativeToBuildDir,
                      const std::string& relativeToRepoRoot) {
    const char* candidates[] = {
        relativeToBuildDir.c_str(),   // CWD == stormhold/port/build(-dist)/
...
fs::path ScratchDirectory() {
    // _dupenv_s rather than std::getenv: MSVC's /W4 flags getenv as C4996,
    // and this project holds a real zero-/W4-warnings bar.
    char* temp = nullptr;
    size_t size = 0;
    _dupenv_s(&temp, &size, "TEMP");
    fs::path base = temp ? fs::path(temp) : fs::path(".");
    free(temp);
    fs::path directory = base / "stormhold_launcher_smoke";
    std::error_code error;
...
int main(int, char**) {
    ...
    std::printf("\n-- 4. game-data discovery --\n");
    {
        ...
        std::ofstream(decoy / "charin.dat") << "not really";
    ...
    // the real TEST-Stormhold.jar.
    std::printf("\n-- 5. installing from a .jar --\n");
    {
        const fs::path jarPath = scratch / "fake.jar";
        Check(WriteTestZip(jarPath.string(),
                           {{"charin.dat", "not really", false},
                            {"itemsin.dat", "nor this, but it inflates", true},
        ...
        Check(ReadWholeText((destination / "itemsin.dat").string()) ==
        ...
        Check(!InstallGameJar(notTheGame.string(), (scratch / "install2").string(), wrongError) &&
                  !wrongError.empty(),
              "a jar that isn't Stormhold is rejected after unpacking");
    }
    ...
    //
    // The real point of this check: this repo's releases endpoint can list
    // more than one game, so the parser has to skip a release that isn't
    // this one's.
    std::printf("\n-- 7. the GitHub release payload, tag-prefix filtered --\n");
    {
        // Two releases in the payload -- a newer dawnstar-v one first, then
        // the stormhold-v one that should actually be picked.
        const std::string json =
            R"([{"tag_name":"dawnstar-v0.2.0","name":"Dawnstar v0.2.0",)"
            R"("draft":false,"prerelease":true,"assets":[)"
            R"({"name":"DawnstarRemastered-dawnstar-v0.2.0-win64.zip",)"
            R"("browser_download_url":)"
            R"("https://github.com/o/r/releases/download/dawnstar-v0.2.0/x.zip"}]},)"
            R"({"tag_name":"stormhold-v0.1.0","name":"Stormhold v0.1.0",)"
            R"("draft":false,"prerelease":true,"assets":[)"
            R"({"name":"checksums.txt","browser_download_url":)"
            R"("https://github.com/o/r/releases/download/stormhold-v0.1.0/checksums.txt"},)"
            R"({"name":"StormholdRemastered-stormhold-v0.1.0-win64.zip",)"
            R"("browser_download_url":)"
            R"("https://github.com/o/r/releases/download/stormhold-v0.1.0/StormholdRemastered-stormhold-v0.1.0-win64.zip"}]}])";
        ReleaseInfo info;
        Check(ParseReleaseList(json, "stormhold-v", info),
              "the stormhold-v release is found despite not being first in the payload");
        Check(info.tag == "stormhold-v0.1.0", "the right tag is read");
        Check(info.version == "0.1.0", "the version drops the stormhold-v prefix");
        Check(info.prerelease, "the prerelease flag is read");
        Check(info.assetName == "StormholdRemastered-stormhold-v0.1.0-win64.zip",
        ...
        ReleaseInfo none;
        Check(!ParseReleaseList(json, "oblivion-v", none),
              "a prefix with no matching release is rejected, not mismatched to another game's");

        ReleaseInfo rejected;
        Check(!ParseReleaseList("[]", "stormhold-v", rejected),
              "an empty release list is rejected");
        Check(!ParseReleaseList("", "stormhold-v", rejected), "an empty body is rejected");
        Check(!ParseReleaseList(R"([{"tag_name":"stormhold-v1.0.0","assets":[]}])", "stormhold-v",
                                rejected),
              "a release with no assets is rejected");
        Check(!ParseReleaseList(
                  R"([{"tag_name":"stormhold-v1.0.0","assets":[{"browser_download_url":)"
                  R"("https://x/y/source.tar.gz"}]}])",
                  "stormhold-v", rejected),
        ...
        ReleaseInfo escaped;
        Check(ParseReleaseList(
                  R"([{"tag_name":"stormhold-v2.0.0","assets":[{"browser_download_url":)"
    ...
    std::printf("\n-- 8. the zip reader --\n");
    {
        Check(IsSafeRelativePath("bin/stormhold_port.exe"), "a normal path is safe");
        ...
        const std::string storedName = "Stormhold.exe";
        const std::string storedBody = "not really an executable";
        const std::string deflateName = "bin/stormhold_port.exe";
        ...
        Check(LooksLikeStormholdPackage(paths), "the archive is recognised as a Stormhold build");
        Check(!LooksLikeStormholdPackage({"readme.txt"}), "an unrelated archive is not");
        Check(!LooksLikeStormholdPackage({"Stormhold.exe"}),
        ...
        Check(ReadWholeText((out / "bin" / "stormhold_port.exe").string()) == deflateBody,
